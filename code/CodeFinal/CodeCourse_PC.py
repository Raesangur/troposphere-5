...
def start_bouge():
    UA = None #Ultrasonic_Avoidance.Ultrasonic_Avoidance(20)
    #fw.turn_straight()
    while True:
        distance = 15 #UA.get_distance()
        if distance < 20:
            time.sleep(0.2)
            start_moving(None, None) #Line_Follower(references=[lineCalibration, lineCalibration, lineCalibration, lineCalibration, lineCalibration]), UA)    
...
def start_moving(lf,UA, vitesseBase = 100):
    ...
    vitesseBase = 100
    cpt = 0
    rd = [False, False, True, False, False] # lf.read_digital()
...
def obstacle_avoidance(lf,UA):
    ...
    rd = [False, True, True, False, False] # lf.read_digital()
...
def get_args(argv):
    ...
    opts, args = getopt.getopt(argv, "hv:s:", ["help", "speed=", "error-speed", "straight=", "delay=", "error-delay=", "line-calibration=", "distance=", "errors=", "errors-max=","distance-duty-cycle="])
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print("\tTroposphere-5: Projet S5 Genie Informatique 66")
            print("\tVoiture SunFounder Picar\n")
            print("\tusage: CodeCourse.py [-h] [--speed <flt>] [--error-speed <uint>] [--straight <uint>] [--delay <flt>] [--error-delay <flt>] [--line-calibration <int>] [--distance <int>] [--errors <uint>] [--errors-max <uint>] [--distance-duty-cycle <uint>]\n")
            ...
            print("\t\t--error-speed\t\t <uint> vitesse de reculon en cas de perte de ligne [0 - 100] (default = " + str(vitesseErreur) + ")")
            print("\t\t--straight\t\t <uint> compteur maximal pour l'acceleration en ligne droite (default = " + str(straightMax) + ")")
            ...
            print("\t\t--line-calibration\t <uint> valeur threshold du suiveur de ligne [0 - 255] (default = " + str(lineCalibration) + ")")
            print("\t\t--distance\t\t <uint> distance en cm entre la voiture et l'obstacle avant de commencer la manoeuvre d'evitement [0 - 50] (default = " + str(obstacleDistance) + ")")
            print("\t\t--errors\t\t <uint> valeur du compteur d'erreur pour commencer a reculer [0 - 1000] (default = " + str(errorThreshold) + ")")
            print("\t\t--errors-max\t\t <uint> valeur maximale du compteur d'erreur [0 - 1000] (default = " + str(errorMax) + ")")
            print("\t\t--distance-duty-cycle\t <uint> Pourcentage de cycle regardant pour un obstacle [0 - 100] (default = " + str(distanceDutyCycle) + ")")
